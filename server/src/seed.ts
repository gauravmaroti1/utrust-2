...
export async function seed() {
  ...
  // ---- Vehicle catalog (idempotent upsert) ----
  const catExists = await one(`SELECT count(*)::int AS c FROM vehicle_catalog`);
  if (!catExists || catExists.c < 1) {
    ...
    for (const r of rows) {
      await query(
        ...
        [r.make, r.model, r.body, r.segment, r.fuels, r.year_from, r.year_to]
      );
    }
    console.log(`[seed] vehicle catalog: ${rows.length} models inserted`);
  }
