  ...
  const e = await one(
    `INSERT INTO evaluations
     (vehicle_id,evaluator_id,branch_id,brand,model,variant,reg_year,kms,owners,fuel_type,
      ...
      ai_assessment,photos,customer_name,reg_no)
    ...
    [b.vehicle_id || null, req.user!.id, req.user!.branch_id, b.brand, b.model, b.variant,
     ...
     b.photos ? JSON.stringify(b.photos) : null, b.customer_name || null, b.reg_no || null]
  );
...
valuationRouter.get("/", async (req: AuthedRequest, res) => {
  ...
  const rows = await query(
    `SELECT e.id,e.brand,e.model,e.variant,e.reg_year,e.reg_no,e.customer_name,e.idv,
            e.condition_score,e.market_value,e.suggested_purchase,e.basis,e.created_at,
...
valuationRouter.get("/:id/pdf", async (req, res) => {
  ...
  const spec: [string, any][] = [
    ...
    ["Owners", e.owners], ["Fuel", e.fuel_type], ["Insurance IDV", e.idv ? INR(e.idv) : "-"],
  ...
  const photos: { name: string; dataUrl: string }[] = Array.isArray(e.photos) ? e.photos : [];
  if (photos.length) {
    ...
    photos.slice(0, 9).forEach((p, i) => {
      ...
      try {
        const b64 = (p.dataUrl || "").replace(/^data:.*;base64,/, "");
