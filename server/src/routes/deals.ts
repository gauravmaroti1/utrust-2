  ...
  const p = await one(
    `INSERT INTO proposals
     (vehicle_id,evaluation_id,evaluator_id,branch_id,proposed_purchase,refurb_estimate,
      expected_resale,gross_margin,roi_pct,status)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'PENDING_MANAGER') RETURNING *`,
    [b.vehicle_id || null, b.evaluation_id || null, req.user!.id, req.user!.branch_id,
     purchase, refurb, resale, margin, Math.round(roi * 100) / 100]
  );
...
proposalRouter.get("/", async (req: AuthedRequest, res) => {
  ...
  res.json(await query(
    `SELECT p.*, us.name AS evaluator_name, v.reg_no, v.make, v.model, b.name AS branch_name
     FROM proposals p
