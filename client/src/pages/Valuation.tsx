...
export default function Valuation() {
  ...
  const [f, setF] = useState<any>({
    brand: "", model: "", variant: "", reg_no: "", chassis_no: "", customer_name: "",
  ...
  const loadHistory = () => api.get("/valuation").then((r) => setHistory(r.data)).catch(() => {});
  ...
  async function save() {
    ...
    setSavedId(data.id); setResult(data); loadHistory();
  }

  async function downloadById(id: number) {
    const res = await api.get(`/valuation/${id}/pdf`, { responseType: "blob" });
    const url = URL.createObjectURL(res.data);
    const a = document.createElement("a"); a.href = url; a.download = `valuation-${id}.pdf`; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ---------------- EVALUATED VEHICLES (record) ---------------- */}
      <div className="card p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-gray-900 dark:text-white">Evaluated Vehicles</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-400 border-b border-gray-100 dark:border-gray-700">
                <th className="py-2">Date</th><th>Vehicle</th><th>Chassis</th><th>IDV</th>
                <th>Cond.</th><th>Market</th><th>Purchase</th><th>Evaluator</th><th></th>
              </tr>
            </thead>
            <tbody>
              {history.length === 0 && (
                <tr><td colSpan={9} className="py-4 text-center text-gray-400">No valuations yet. Calculate and save one above.</td></tr>
              )}
              {history.map((h) => (
                <tr key={h.id} className="border-b border-gray-50 dark:border-gray-700/50">
                  <td className="py-2 text-gray-500">{String(h.created_at).slice(0, 10)}</td>
                  <td className="font-medium text-gray-900 dark:text-white">{h.brand} {h.model} <span className="text-gray-400">{h.variant} {h.reg_year}</span></td>
                  <td className="text-gray-500">{h.chassis_no ? `…${String(h.chassis_no).slice(-5)}` : "—"}</td>
                  <td className="text-gray-500">{h.idv ? INR(h.idv) : "—"}</td>
                  <td className="text-gray-600 dark:text-gray-300">{h.condition_score ?? "—"}</td>
                  <td className="text-gray-500">{INR(h.market_value)}</td>
                  <td className="font-medium text-prakash-red">{INR(h.suggested_purchase)}</td>
                  <td className="text-gray-500">{h.evaluator_name}</td>
                  <td><button onClick={() => downloadById(h.id)} className="text-xs text-prakash-red hover:underline">PDF</button></td>
                </tr>
              ))}
            </tbody>
          </table>
